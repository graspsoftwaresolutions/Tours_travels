                ...
                <div class="paper toolbar-parent mt10">
                    <div class="p10 theme dropshadow mb4">
                        <div class="toolbar">
                            ...
                            <h3 class="title medium">{{__('Room Type List') }}</h3>
                        </div>
                    ...
                    <div class="collapsible-target">
                        ...
                        <table id="datatable-master" class="table-datatable dt-responsive table-striped table-hover">
                            <thead>
                                <tr>
                                    <th width="80%">{{__('Room Type Name') }}</th>

                                    <th> {{__('Action') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                            @foreach($data['room_type'] as $values)
                                <tr>
                                ...
                                <td>
                                        ...
                                        &nbsp;
                                        <a>
                                            <form style='display:inline-block;' action='{{ route("master.roomtypedestroy",$values->id) }}' method='POST'>
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
            ...
            <div id="masterModal" class="modal" tabindex="-1" role="dialog">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <form class="formValidate" id="roomTypeformValidate" method="post" action="{{ route('master.saveroomtype') }}">
                            @csrf
...
<script type="text/javascript">
    ...
    // Form Validation
    $("#roomTypeformValidate").validate({
        rules: {
            room_type: {
                required: true,
                remote: {
                    url: "{{ route('room_type_nameexists')}}",
                    data: {
                        roomtype_id: function() {
                            return $("#masterid").val();
                        },
                        _token: "{{csrf_token()}}",
        ...
        messages: {
            room_type: {
                required: '{{__("Please enter Room Type Name") }}',
                 remote: '{{__("Room Type Name Already exists") }}',
            }
        },
    });
    function showeditForm(roomtypeid) {
        $("#masterModal").modal();
        
        var url = "{{ route('roomtype_detail') }}" + '?roomtype_id=' + roomtypeid;
        $.ajax({
            url: url,
            type: "GET",
